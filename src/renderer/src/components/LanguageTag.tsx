...
export function LanguageTag({
  ...
  const style = STYLES[lang as Exclude<DetectedLanguage, 'unknown'>]
  if (style) {
    return (
      <span
  ...
  // Engine LID tags beyond zh/en (yue/ja/ko/fr…) — neutral badge
  return (
    <span
      className={`mr-1.5 inline-flex shrink-0 translate-y-[-1px] items-center rounded border border-slate-400/40 bg-slate-500/15 px-1 py-px font-mono text-[10px] font-semibold tracking-wide text-slate-300 ${className}`}
      style={{ lineHeight: 1 }}
      title={`引擎识别语言：${lang}`}
    >
